...
最新正式版：[Cohub 2.1.0](https://github.com/atou42/cohub-skill/releases/tag/cohub-v2.1.0)。[versions.json](versions.json) 指向已核实的发布标签与提交；正式安装使用该标签，不使用可能更新的 main 工作树。
...
安装本正式版时，指定 cohub-v2.1.0 标签下的 zh-CN/skills/cohub 目录；仅在明确试用开发版时使用 main。完整保留 references、scripts 和 version.json。已有安装、语言切换及旧 App Developer 迁移见[安装与迁移](zh-CN/skills/cohub/references/安装与迁移.md)。不会未经授权删除旧安装。
