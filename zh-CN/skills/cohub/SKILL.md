...
本包为 Cohub 2.1.1 简体中文说明版。说明语言不决定回复语言；继续遵循用户当前语言和已有偏好，除非用户明确要求切换。首次安装、升级或切换语言时读取[安装与迁移](references/安装与迁移.md)，报告版本、说明语言和实际安装位置。只保留一个活跃的 cohub 安装，不同时启用中英文版或旧的独立 cohub-app-developer。
...
只读取当前步骤需要的文件，不递归加载所有链接。每次成功发布的结果交付（包括更新已有版本）都必须输出发布卡；卡片说明在交付时才读取；定义、枚举和模板不预先加载。当前上下文中已读且未变化的说明直接复用。

- 使用 Style、Game Maker、Fandom、OKP 搜索或 Character Traits 时，按下方能力表直接读取来源入口。
- 向 Space 工作区上传 100 个及以上文件，或零散文件上传明显缓慢：读[批量上传](references/批量上传.md)，优先考虑打包上传后解压。
